return {
  ...
  config = function()
    ...
    lsp_zero.on_attach(function(_, buffer)
      ...
      map("n", "gs", "<cmd>lua vim.lsp.buf.signature_help()<cr>", "Signature Help", buffer)
      ...
      map("n", "[d", "<cmd>lua vim.diagnostic.goto_prev()<cr>", "Prev Diagnostic", buffer)
      map("n", "]d", "<cmd>lua vim.diagnostic.goto_next()<cr>", "Next Diagnostic", buffer)
      map("n", "<leader>cR", "<cmd>e<cr>", "Reload Lsp", buffer)
      --stylua: ignore end
    end)
